...
#[derive(Parser)]
#[command(name = "macmon")]
#[command(version = "4.0.3", about = "OmniMon: Monitor de sistema y navegador de próxima generación de alto rendimiento.", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
    ...
    match &cli.command {
        Commands::Status { format } => {
            ...
            match format {
                Format::Json => {
                    println!(
                        "{{\"status\":\"running gracefully\",\"memory_usage_bytes\":{}}}",
                        mock_memory_bytes
                    );
                }
                Format::Text => {
